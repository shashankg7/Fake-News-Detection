...
for line in buzz_network:
	...
	temp = map(lambda x:(int(x)-1), line.strip().split())
	buzz_adjmat[temp[0], temp[1]] = 1
	buzz_one_indices.append((temp[0], temp[1]))
	buzz_y.append(1)
...
for line in poli_network:
	...
	temp = map(lambda x:(int(x)-1), line.strip().split())
	poli_adjmat[temp[0], temp[1]] = 1
	poli_one_indices.append((temp[0], temp[1]))
	poli_y.append(1)
...
buzz_test_y = np.array(buzz_test_y)
zero_indices = np.where(buzz_test_y == 1)
buzz_test_y = buzz_test_y[zero_indices]
buzz_test_feats = buzz_test_feats[zero_indices]
pdb.set_trace()
...
print "Acc for Buzz %f"%(accuracy_score(buzz_test_y, y_pred))



poli_test_y = np.array(poli_test_y)
zero_indices = np.where(poli_test_y == 1)
poli_test_y = poli_test_y[zero_indices]
poli_test_feats = poli_test_feats[zero_indices]
...
print "Acc for politifact %f"%(accuracy_score(poli_test_y, y_pred))
