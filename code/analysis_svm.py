...
def baseline_ngram_svm(X_buzz, y_buzz, X_poli, y_poli):
	...
	for i in range(10):	
		...
		sc += metrics.calinski_harabaz_score(X_buzz_ngram.todense(), km.labels_)
		
	
	print "Silhouette Coefficient for BuzzFeed dataset: %.3f"%(sc/float(10))
	sc = 0.0

	for i in range(10):
		km.fit(X_poli_ngram)
		#sc += metrics.silhouette_score(X_poli_ngram, km.labels_)
		sc += metrics.calinski_harabaz_score(X_poli_ngram.todense(), km.labels_)

	print "Silhouette Coefficient for PolitiFact dataset: %.3f"%(sc/float(10))
