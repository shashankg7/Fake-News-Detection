...
def parse_newsdata():
	...
	for dataset in datasets:
		...
		#svd_model = TruncatedSVD(n_components=45)
...
def baseline_lsi_svm(X_buzz, y_buzz, X_poli, y_poli):
	...
	for i in range(10):
		...
		sc += metrics.calinski_harabaz_score(X_buzz_ngram, km.labels_)

	print "Silhouette Coefficient %.3f"%(sc/float(10))
	sc = 0.0
	for i in range(10):
		km.fit(X_poli_ngram)
		#sc += metrics.silhouette_score(X_poli_ngram, km.labels_)
		sc += metrics.calinski_harabaz_score(X_poli_ngram, km.labels_)
